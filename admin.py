...
def panel():
    ...
    if current_user.is_owner:
        ...
        base_query = User.query.filter_by(is_seed_data=False)
        total_admins = ADMIN_VISIBLE_ADMIN_COUNT
    ...
    if current_user.is_owner:
        ...
        for u in page_users:
            if u.is_owner:
                rows.append({"user": u, "role_label": "Owner", "badge": True, "role_class": "role-owner"})
            elif u.id == current_user.id:
                rows.append({"user": u, "role_label": "Queen", "badge": True, "role_class": "role-queen"})
            else:
...
def diagnostics():
    ...
    if current_user.is_owner:
        ...
        db_info["real_users"] = User.query.filter_by(is_seed_data=False).count()
        db_info["demo_users"] = User.query.filter_by(is_seed_data=True).count()
